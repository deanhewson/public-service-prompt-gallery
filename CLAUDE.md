...
- **Agent-ready schema (Sprint 1):** `agent_mode` (per step: `autonomous` | `human_gate`), `agent_export` (per workflow), and `tool_augmented` (per quality gate, optional) — see `CREATING-WORKFLOWS.md` for structure. Agent exports are **Experimental** — `agent_export` is a per-platform flag entirely separate from `maturity`; never merge or derive one from the other. Optional steps (`is_optional: true`) are included by default in agent execution.
- **Agent export generator (Sprint 2):** `scripts/export-agent-skills.js` reads `data/workflows/`, `data/building-blocks/`, and `data/quality-gates/` (read-only) and writes to `public/agent-skills/`: for each workflow with `agent_export.enabled === true`, a `<slug>-claude-skill.md`, `<slug>-copilot-agent.md`, and `<slug>-agent-instructions.md`, plus a shared `index.json` manifest. Runs automatically as `npm run build`'s `prebuild` step, or standalone via `npm run export-skills`. Output is deterministic — re-running produces byte-identical files (the only date in the output is `maturity_updated`/`last_reviewed`, never the clock). Copilot exports must fit Agent Builder's limits: description ≤ 1,000 chars, instructions ≤ 8,000 chars — the script exits non-zero and names the offending workflow if exceeded. `public/agent-skills/**` is committed (matching the existing `public/favicon.*` convention), not gitignored.
